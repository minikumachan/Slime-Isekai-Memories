...
function calculateDamage() {
  ...
  try {
    ...
    renderGraph({ '通常ダメージ': lastTotalResult.skillFinal, '奥義ダメージ': lastTotalResult.ultimateFinal });
  } catch (error) {
...
function openComparisonModal() {
  ...
  content.innerHTML = '<h2>比較するバフを選択</h2><p>ここでチェックした項目はバフ適応前と適応後のそれぞれのダメージが再計算され比較されます。</p>';
...
function updateComparisonUI() {
  $('#show-comparison-graph-btn').toggle(isComparisonModeActive);
...
function openComparisonGraphModal() {
  if (!isComparisonModeActive || !lastTotalResult || !lastComparisonResult) {
    alert('比較データがありません。先に比較を適用してください。');
    return;
  }
  const existingModal = document.getElementById('comparison-graph-modal');
  if (existingModal) existingModal.remove();
  const modal = document.createElement('div');
  modal.id = 'comparison-graph-modal';
  ...
  let html = '<h2>ダメージ比較グラフ</h2>';

  // ★★【変更点】メイン画面のトグルボタンの状態を取得 ★★
  const showCrit = c('toggle-crit');
  const showPierce = c('toggle-pierce');
  const showSynergy = c('toggle-synergy');
  const showKensan = c('toggle-kensan');
  ...
  const sections = [
    { title: '奥義ダメージ', totalData: lastTotalResult.ultimateFinal, compData: lastComparisonResult.ultimateFinal },
    { title: '通常ダメージ', totalData: lastTotalResult.skillFinal, compData: lastComparisonResult.skillFinal }
  ];

  // ★★【変更点】表示するダメージタイプをトグル状態に応じてフィルタリング ★★
  const damageTypes = {
    normal: { name: '通常' },
    ...(showSynergy && { synergy: { name: '協心' } }),
    ...(showKensan && { kensan: { name: '堅閃' } }),
    ...(showSynergy && showKensan && { synergyKensan: { name: '協心+堅閃' } })
  };

  sections.forEach(section => {
    html += `<h3 style="margin-top: 2rem; color: #9efaad;">▼ ${section.title}</h3>`;
    const allValues = [];
    Object.keys(damageTypes).forEach(type => {
      allValues.push(section.totalData[type], section.compData[type]);
      const expectedTotal = calculateExpectedDamage(section.totalData[type], section.totalData[type] * lastTotalResult.critTTL, (section.totalData[type] + lastTotalResult.displayAttack * 0.06) * lastTotalResult.pierceTTL, (section.totalData[type] * lastTotalResult.critTTL + lastTotalResult.displayAttack * 0.06) * lastTotalResult.pierceTTL, lastTotalResult.critRate, lastTotalResult.pierceRate);
      allValues.push(expectedTotal);
      const expectedComp = calculateExpectedDamage(section.compData[type], section.compData[type] * lastComparisonResult.critTTL, (section.compData[type] + lastComparisonResult.displayAttack * 0.06) * lastComparisonResult.pierceTTL, (section.compData[type] * lastComparisonResult.critTTL + lastComparisonResult.displayAttack * 0.06) * lastComparisonResult.pierceTTL, lastComparisonResult.critRate, lastComparisonResult.pierceRate);
      allValues.push(expectedComp);
    });
    const maxValue = Math.max(...allValues, 1);

    Object.keys(damageTypes).forEach(type => {
      const { name } = damageTypes[type];
      const totalDamage = section.totalData[type];
      const compDamage = section.compData[type];
      const expectedTotal = calculateExpectedDamage(totalDamage, totalDamage * lastTotalResult.critTTL, (totalDamage + lastTotalResult.displayAttack * 0.06) * lastTotalResult.pierceTTL, (totalDamage * lastTotalResult.critTTL + lastTotalResult.displayAttack * 0.06) * lastTotalResult.pierceTTL, lastTotalResult.critRate, lastTotalResult.pierceRate);
      const expectedComp = calculateExpectedDamage(compDamage, compDamage * lastComparisonResult.critTTL, (compDamage + lastComparisonResult.displayAttack * 0.06) * lastComparisonResult.pierceTTL, (compDamage * lastComparisonResult.critTTL + lastComparisonResult.displayAttack * 0.06) * lastComparisonResult.pierceTTL, lastComparisonResult.critRate, lastComparisonResult.pierceRate);

      html += `<div style="margin: 1.5rem 0;"><h4>${name} (基礎)</h4>
                     <div class="graph-item"><div class="graph-label" style="color:#a9dff9;">合計</div><div class="graph-bar-container"><div class="graph-bar" style="width: ${totalDamage / maxValue * 100}%; background: linear-gradient(90deg, #54ebf9, #9efaad);">${Math.round(totalDamage).toLocaleString()}</div></div></div>
                     <div class="graph-item"><div class="graph-label" style="color:#ffb899;">バフ無し</div><div class="graph-bar-container"><div class="graph-bar" style="width: ${compDamage / maxValue * 100}%; background: linear-gradient(90deg, #ff7e5f, #feb47b);">${Math.round(compDamage).toLocaleString()}</div></div></div>
                   </div>`;

      // ★★【変更点】期待値グラフの表示条件を更新 ★★
      if ((showCrit || showPierce) && (lastTotalResult.critRate > 0 || lastTotalResult.pierceRate > 0)) {
        html += `<div style="margin: 1.5rem 0;"><h4>${name} (期待値)</h4>
                     <div class="graph-item"><div class="graph-label" style="color:#a9dff9;">合計</div><div class="graph-bar-container"><div class="graph-bar" style="width: ${expectedTotal / maxValue * 100}%; background: linear-gradient(90deg, #54ebf9, #9efaad);">${Math.round(expectedTotal).toLocaleString()}</div></div></div>
                     <div class="graph-item"><div class="graph-label" style="color:#ffb899;">バフ無し</div><div class="graph-bar-container"><div class="graph-bar" style="width: ${expectedComp / maxValue * 100}%; background: linear-gradient(90deg, #ff7e5f, #feb47b);">${Math.round(expectedComp).toLocaleString()}</div></div></div>
                   </div>`;
      }
    });
  });
...
$(function () {
  ...
  $('body').on('click', '.toggle-btn', function () {
    ...
    calculateDamage(); // これも計算をトリガーするので自動保存される
  });

  $('#clear-inputs').on('click', () => clearAllInputs(true));
  $('#open-compare-modal-btn').on('click', openComparisonModal);
  $('#show-comparison-graph-btn').on('click', openComparisonGraphModal);
...
function exportPresets() { let filename = prompt("書き出すファイル名を入力してください:", "maoryu-presets.json"); if (!filename || filename.trim() === "") { alert("エクスポートをキャンセルしました。"); return; } if (!filename.toLowerCase().endsWith('.json')) filename += '.json'; if (confirm(`ファイル名「${filename}」でプリセットを書き出しますか？`)) { const exportData = { skillPresets: getStoredJSON(SKILL_PRESET_KEY), dataPresets: {} }; for (let i = 1; i <= MAX_DATA_SLOTS; i++) { const data = getStoredJSON(`${DATA_PRESET_KEY_PREFIX}${i}`, null); if (data) exportData.dataPresets[i] = data; } const jsonString = JSON.stringify(exportData, null, 2); const blob = new Blob([jsonString], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = filename; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); } else { alert("エクスポートをキャンセルしました。"); } }
function importPresets(event) { const file = event.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = (e) => { try { const importedData = JSON.parse(e.target.result); if (!importedData.skillPresets && !importedData.dataPresets) throw new Error('無効なファイル形式です。'); if (confirm('現在のプリセットを上書きしてインポートしますか？この操作は元に戻せません。')) { if (Array.isArray(importedData.skillPresets)) localStorage.setItem(SKILL_PRESET_KEY, JSON.stringify(importedData.skillPresets)); Object.keys(localStorage).forEach(key => { if (key.startsWith(DATA_PRESET_KEY_PREFIX)) localStorage.removeItem(key); }); if (importedData.dataPresets) { for (const i in importedData.dataPresets) { if (i >= 1 && i <= MAX_DATA_SLOTS) localStorage.setItem(`${DATA_PRESET_KEY_PREFIX}${i}`, JSON.stringify(importedData.dataPresets[i])); } } alert('プリセットのインポートが完了しました。'); renderSkillPresetEditor(); renderSkillPresetActivator(); renderDataPresetButtons(); calculateDamage(); } } catch (error) { alert(`インポートに失敗しました: ${error.message}`); } finally { event.target.value = null; } }; reader.readAsText(file); }
...
function renderGraph(results) { const container = document.getElementById('graph-container'); if (!container) return; let graphHTML = '<h3>ダメージグラフ</h3><div class="graph">'; const allDamages = []; for (const title in results) { const data = results[title]; for (const type in data) { if (data[type] > 0) { let label = ''; switch (type) { case 'normal': label = '通常'; break; case 'synergy': label = '協心'; break; case 'kensan': label = '堅閃'; break; case 'synergyKensan': label = '協心+堅閃'; break; } if (title === '奥義ダメージ') label += '(奥義)'; allDamages.push({ label: label, value: data[type] }); } } } if (allDamages.length > 0) { const maxValue = Math.max(...allDamages.map(d => d.value)); allDamages.forEach(item => { const barWidth = (item.value / maxValue) * 100; graphHTML += `<div class="graph-item"><div class="graph-label" title="${item.label}">${item.label}</div><div class="graph-bar-container"><div class="graph-bar" style="width: ${barWidth}%;">${Math.round(item.value).toLocaleString()}</div></div></div>`; }); } else { graphHTML += '<p>計算結果がありません。</p>'; } graphHTML += '</div>'; container.innerHTML = graphHTML; }
